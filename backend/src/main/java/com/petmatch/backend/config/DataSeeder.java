...
public class DataSeeder implements CommandLineRunner {
    ...
    private void seedDefaultAdmin() {
        ...
        User admin = User.builder()
                ...
                .build();
        userRepo.save(admin);
        log.info("DataSeeder: Đã tạo tài khoản admin mặc định: admin@petmatch.com / Admin@123");
    }
    ...
    private void seedSampleReports() {
        ...
        List<PetProfile> pets = petProfileRepo.findAll();
        String[] reasons = {
                "Ảnh phản cảm",
                "Nội dung spam",
                "Thông tin sai sự thật",
                "Quấy rối người dùng khác",
                "Dùng ảnh không phải thú cưng"
        };

        for (int i = 0; i < 20; i++) {
            User reporter = users.get(RNG.nextInt(users.size()));

            boolean targetPet = !pets.isEmpty() && RNG.nextBoolean();
            Report report;
            if (targetPet) {
                PetProfile target = pets.get(RNG.nextInt(pets.size()));
                if (target.getOwner().getId().equals(reporter.getId())) continue;
                report = Report.builder()
                        .reporter(reporter)
                        .targetType(ReportTargetType.PET_PROFILE)
                        .targetId(target.getId())
                        .reason(reasons[RNG.nextInt(reasons.length)])
                        .status(ReportStatus.PENDING)
                        .build();
            } else {
                User target = users.get(RNG.nextInt(users.size()));
                if (target.getId().equals(reporter.getId())) continue;
                report = Report.builder()
                        .reporter(reporter)
        ...
        log.info("DataSeeder: Đã seed thêm báo cáo vi phạm để test admin moderation.");
    }
